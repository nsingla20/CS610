...
inline void gridloopsearch(double b[30], double a[120], double kk) {
  ...
  __attribute__((aligned(16))) double c[12][12];
  ...
  int si = floor((b[13] - b[12]) / b[14]);
  // double r1, r2, r3, r4, r5, r6, r7, r8, r9, r10;

  omp_set_nested(1);
  // grid search starts
  #pragma omp parallel for default(none) shared(c,b,fptr,s,si,e) private(qb) reduction(+:pnts) num_threads(4)
    ...
    for (double r2 = b[3]; r2 < b[4]; r2+=b[5]) {
      ...
      for (double r3 = b[6]; r3 < b[7]; r3+=b[8]) {
        ...
        for (double r4 = b[9]; r4 < b[10]; r4+=b[11]) {
          ...
          #pragma omp parallel for default(none) shared(c,b,fptr,r1,r2,r3,r4,s,si,e) firstprivate(qb) reduction(+:ans) num_threads(4)
          for (int s5 = 0; s5 < si; s5++) {
            double r5 = b[12] + s5*b[14];
